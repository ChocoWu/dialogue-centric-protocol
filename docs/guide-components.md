...
A **DCP component** is a *versioned capability or definition* — a control policy (orchestrator),
oversight policy, model provider, **agent**, or dialogue template — described by a machine-readable
**manifest** and delivered one of several ways: as a local Python package, a local package plus a
model checkpoint, or a **remote service** you connect to. DCP gives you one pipeline to
**describe → resolve → provision → run** a component wherever it actually lives.

> **Component vs. plugin.** [guide-sharing.md](guide-sharing.md) covers the quick in-process path:
...
A component ships a `dcp-component.yaml`. Identity, interface, and the kind-specific `spec` are
separate blocks:
...
**Kinds** map to a runtime interface: `control_policy → ControlPolicy`, `oversight_policy →
OversightPolicy`, `model_provider → ModelProvider`, `agent → AgentDefinition` (see §4c),
`template → DialogueTemplate`. Templates are *declarative* — the rest are *executable*.
...
later stages. The `dcp` CLI exposes each step. A reference is a `file://` path or `installed://name`:
...
or `Orchestrator(..., control_policy=…)` (see [guide-extending.md](guide-extending.md)).
...
then calls the entrypoint as `factory(checkpoint_path)`. `hf://` needs the extra `pip install
'dcp[hf]'`; `file://` and `https://` artifacts need no extra.
...
Host a **payload handler** (the wire contract is the projected payload, not a Python type) and run it
with uvicorn:
...
A remote **agent** connects the same way — `connect` returns an `AgentDefinition` (see §4c), which is
also a drop-in `agent_provider`:
...
`AgentDefinition` also *acts as* a `ModelProvider` (it delegates `text`/`structured`), which is why
it drops straight into `agent_providers`.
...
- **Owner-controlled projection (D12):** *you* decide what leaves your boundary with a
  ...
  dialogue's event log (`CONTEXT_PROJECTED`, with a digest).
...
- [guide-sharing.md](guide-sharing.md) — the in-process entry-point plugin path (the quick way).
- [guide-extending.md](guide-extending.md) — *authoring* the interfaces a component wraps.
